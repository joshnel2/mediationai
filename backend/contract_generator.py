...
_contract_generator = None

def get_contract_generator():
    """Get the global contract generator instance (lazy initialization)"""
    global _contract_generator
    if _contract_generator is None:
        _contract_generator = ContractGenerator()
    return _contract_generator

# Create a property-like object for backwards compatibility
class ContractGeneratorProxy:
    def __getattr__(self, name):
        return getattr(get_contract_generator(), name)

contract_generator = ContractGeneratorProxy()